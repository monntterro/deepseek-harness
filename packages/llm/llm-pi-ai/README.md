...
A catalog-based selector shows the model list bundled at build time, which is why a provider that adds or retires a model is invisible until a newer pi-ai ships, or until someone edits `settings.yaml`. Setting `refreshCatalog: true` on a route overlays that route's OpenAI-compatible `GET /models` listing over the installed catalog on **every** discovery read (`ctx.llm.listModels` — the Web model picker and the ACP editors each read it on open), so the selector reflects the provider's current lineup instead of the static one.

The overlay is the provider's current lineup, so it diverges from the installed catalog three ways: an installed id the endpoint still advertises keeps its authored metadata (context window, compat, reasoning) — the listing endpoint reports none of that, and overwriting a rich catalog entry with a bare row would lose it; an id the endpoint advertises but the installed catalog does not becomes a servable descriptor inheriting the route's OpenAI template (protocol, endpoint, compat, reasoning), so selecting it dispatches rather than failing `UNKNOWN_MODEL`; an installed id the endpoint no longer advertises is dropped, so a model the provider retired does not linger in the selector.

`refreshCatalog` needs an OpenAI-compatible listing origin. It is derived from the route's `baseURL` when set, else from the `baseUrl` of the route's first OpenAI-compatible baseline model; a route that names neither — a non-OpenAI catalog provider with no `baseURL` — cannot be derived and resolution refuses it rather than inventing an endpoint. A failed live listing is swallowed: the static baseline already loaded keeps serving and the refresh error is logged (`serving the static catalog`), so a transient endpoint outage never empties the selector. The refresh is in-memory per process and refetches each open, so there is no stale on-disk overlay and no freshness window to configure.

`refreshCatalog` has no effect when a non-empty explicit `models` list curates the route: an explicit list is the deployment's hand-maintained lineup, which an endpoint listing would replace, so the static behavior stands. A `modelOverrides`-only route still overlays, because that route still serves its installed catalog.
...
- **A route's catalog is static unless `refreshCatalog` opts in** — without `refreshCatalog: true`, the catalog is whatever `settings.yaml` says, so a model list is only as current as its last edit and nothing queries a provider for its lineup. With `refreshCatalog`, the selector overlays the endpoint's `GET /models` on every discovery read (see [Live catalog refresh](#live-catalog-refresh)); a failed listing falls back to the static catalog rather than emptying the selector.
